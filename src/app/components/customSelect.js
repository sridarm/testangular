(function() {
    ...
    angular.module('testangular').directive('customSelect', customSelect);
    angular.module('testangular').directive('stopPropagation', stopPropagation);
    angular.module('testangular').value('customSelectDefaults', {
        displayText: 'Select...',
        popoverContent: 'Testing popover',
        emptyListText: 'There are no items to display',
        emptySearchResultText: 'No results match "$0"',
        addText: 'Add',
        searchDelay: 0
    });
    angular.module('testangular').directive('popoverHtmlUnsafePopup', popoverHtmlUnsafePopup);
    angular.module('testangular').directive('popoverHtmlUnsafe', popoverHtmlUnsafe);
    ...
    function popoverHtmlUnsafePopup() {
        return {
            ...
            templateUrl: "app/components/popover-html-unsafe-popup.html"
        };
    }
    ...
    function customSelect($parse, $compile, $timeout, $q, customSelectDefaults) {
        ...
        return {
            ...
            link: function(scope, elem, attrs, controller) {
                ...
                var itemTemplate = elem.html().trim() || '{{' + (match[2] || match[1]) + '}}',
                    dropdownTemplate = '<a class="dropdown-toggle" data-toggle="dropdown" href ng-class="{ disabled: disabled }">' + '<span>{{displayText}}</span>' + '<b></b>' + '</a>' + '<div class="dropdown-menu">' + '<div stop-propagation="click" class="custom-select-search">' + '<input class="' + attrs.selectClass + '" type="text" autocomplete="off" ng-model="searchTerm" />' + '</div>' + '<ul role="menu">' + '<li role="presentation" ng-repeat="' + valueName + ' in matches" popover-html-unsafe="{{popoverContent(' + valueName + ')}}">' + '<a role="menuitem" tabindex="-1" href ng-click="select(' + valueName + ')">' + itemTemplate + '</a>' + '</li>' + '<li ng-hide="matches.length" class="empty-result" stop-propagation="click">' + '<em class="muted">' + '<span ng-hide="searchTerm">{{emptyListText}}</span>' + '<span class="word-break" ng-show="searchTerm">{{ format(emptySearchResultText, searchTerm) }}</span>' + '</em>' + '</li>' + '</ul>' + '<div class="custom-select-action">' + (typeof options.onAdd === "function" ? '<button type="button" class="btn btn-primary btn-block add-button" ng-click="add()">{{addText}}</button>' : '') + '</div>' + '</div>';
                ...
                childScope.$watch(getPopoverText, setPopoverContent);
                ...
                function setPopoverContent() {
                    ...
                    needsDisplayText = !text;
                    childScope.popoverContent = text || options.popoverContent;
                }

                function getOptions() {
                    return angular.extend({}, customSelectDefaults, scope.$eval(attrs.customSelectOptions));
                }
